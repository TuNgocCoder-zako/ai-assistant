...
1. [`dataset.json`](dataset.json): Tập dữ liệu 450 mẫu hội thoại chuẩn tiếng Việt đời thường, súc tích 1-2 câu.
2. [`generate_dataset.py`](generate_dataset.py): Script mở rộng hoặc bổ sung thêm dữ liệu hội thoại mới.
3. [`Train_Alexa_Llama3_2_Colab.ipynb`](Train_Alexa_Llama3_2_Colab.ipynb): Jupyter Notebook hoàn chỉnh để chạy huấn luyện trên Google Colab GPU T4 miễn phí.
4. [`Modelfile.template`](Modelfile.template): Mẫu cấu hình nạp mô hình sau khi train vào Ollama.
...
2. Chọn **Upload (Tải lên)** -> Chọn tệp [`Train_Alexa_Llama3_2_Colab.ipynb`](Train_Alexa_Llama3_2_Colab.ipynb).
...
2. Tại **Bước 3 (Upload dataset)**, Colab sẽ hiện nút chọn tệp: Hãy chọn tệp [`dataset.json`](dataset.json).
...
mv ~/Downloads/*.gguf /home/tu/Projects/python/voice-ai/training/alexa-trained.gguf

# Đăng ký mô hình mới vào Docker Ollama
docker exec -it fb718d88864a ollama create alexa-vn-pro -f /tmp/Modelfile
```

Sau đó trong [`assistant.py`](../assistant.py), bạn chỉ cần đổi tên mô hình:
```python
